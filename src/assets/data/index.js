import bestImg3 from "../img/bestImg.png";
import bestImg1 from "../img/bestImg2.png";
import bestImg2 from "../img/bestImg3.png";
import zommer from "../img/zoomer.png";
import shells from "../img/shells.png";
import artVenue from "../img/art-venue.png";
import wa from "../img/wa.png";
import congue from "../img/Congue.png";
import ut from "../img/Ut.png";
import aliquam from "../img/Aliquam.png";
import fusce from "../img/Fusce.png";
import ipsum from "../img/Ipsum.png";
import purus from "../img/Purus.png";
import urna from "../img/Urna.png";
import vitae from "../img/Vitae.png";
import { logoIcon } from "../../components/svgs/logo";



export const links = ["Products", "Solutions", "Pricing", "Resources", "Log In"];
export const bestData = [
    { img: bestImg1, title: "Title", text: "Egestas elit dui scelerisque ut eu purus aliquam vitae habitasse." },
    { img: bestImg2, title: "Title", text: "Egestas elit dui scelerisque ut eu purus aliquam vitae habitasse." },
    { img: bestImg3, title: "Title", text: "Egestas elit dui scelerisque ut eu purus aliquam vitae habitasse." }
]

export const comments = [
    { compName: "Zoomerr", text: "Non risus viverra enim, quis. Eget vitae arcu vivamus sit tellus, viverra turpis lorem. Varius a turpis urna id porttitor.", logo: logoIcon.ZoomerIcon, avatar: zommer, name: "Hellen Jummy", jobTitle: "Team Lead" },
    { compName: "SHELLS", text: "Aliquet ridiculus mi porta habitant vulputate rhoncus, mattis amet enim. Sit purus venenatis velit semper lectus sed ornare quam nulla.", logo: logoIcon.ShelsIcon, avatar: shells, name: "Hellena John", jobTitle: "Co-founder" },
    { compName: "ArtVenue", text: "A eget sed posuere dui risus habitasse mauris. Venenatis aliquet id ultrices a lacus. Pretium vehicula pretium posuere justo sed.", logo: logoIcon.ArtVenueIcon, avatar: artVenue, name: "David Oshodi", jobTitle: "Manager" },
    { compName: "WA", text: "Magna egestas aliquet ut integer non. Sed diam enim nibh sit. Aliquam laoreet aenean metus nibh eu scelerisque.", logo: logoIcon.WaIcon, avatar: wa, name: "name", jobTitle: "job" },
    { compName: "Zoomerr", text: "Non risus viverra enim, quis. Eget vitae arcu vivamus sit tellus, viverra turpis lorem. Varius a turpis urna id porttitor.", logo: logoIcon.ZoomerIcon, avatar: zommer, name: "Hellen Jummy", jobTitle: "Team Lead" },
    { compName: "SHELLS", text: "Aliquet ridiculus mi porta habitant vulputate rhoncus, mattis amet enim. Sit purus venenatis velit semper lectus sed ornare quam nulla.", logo: logoIcon.ShelsIcon, avatar: shells, name: "Hellena John", jobTitle: "Co-founder" },
    { compName: "ArtVenue", text: "A eget sed posuere dui risus habitasse mauris. Venenatis aliquet id ultrices a lacus. Pretium vehicula pretium posuere justo sed.", logo: logoIcon.ArtVenueIcon, avatar: artVenue, name: "David Oshodi", jobTitle: "Manager" },
    { compName: "WA", text: "Magna egestas aliquet ut integer non. Sed diam enim nibh sit. Aliquam laoreet aenean metus nibh eu scelerisque.", logo: logoIcon.WaIcon, avatar: wa, name: "name", jobTitle: "job" },
    { compName: "Zoomerr", text: "Non risus viverra enim, quis. Eget vitae arcu vivamus sit tellus, viverra turpis lorem. Varius a turpis urna id porttitor.", logo: logoIcon.ZoomerIcon, avatar: zommer, name: "Hellen Jummy", jobTitle: "Team Lead" },
    { compName: "SHELLS", text: "Aliquet ridiculus mi porta habitant vulputate rhoncus, mattis amet enim. Sit purus venenatis velit semper lectus sed ornare quam nulla.", logo: logoIcon.ShelsIcon, avatar: shells, name: "Hellena John", jobTitle: "Co-founder" },
    { compName: "ArtVenue", text: "A eget sed posuere dui risus habitasse mauris. Venenatis aliquet id ultrices a lacus. Pretium vehicula pretium posuere justo sed.", logo: logoIcon.ArtVenueIcon, avatar: artVenue, name: "David Oshodi", jobTitle: "Manager" },
    { compName: "WA", text: "Magna egestas aliquet ut integer non. Sed diam enim nibh sit. Aliquam laoreet aenean metus nibh eu scelerisque.", logo: logoIcon.WaIcon, avatar: wa, name: "name", jobTitle: "job" },
    { compName: "Zoomerr", text: "Non risus viverra enim, quis. Eget vitae arcu vivamus sit tellus, viverra turpis lorem. Varius a turpis urna id porttitor.", logo: logoIcon.ZoomerIcon, avatar: zommer, name: "Hellen Jummy", jobTitle: "Team Lead" },
    { compName: "SHELLS", text: "Aliquet ridiculus mi porta habitant vulputate rhoncus, mattis amet enim. Sit purus venenatis velit semper lectus sed ornare quam nulla.", logo: logoIcon.ShelsIcon, avatar: shells, name: "Hellena John", jobTitle: "Co-founder" },
    { compName: "ArtVenue", text: "A eget sed posuere dui risus habitasse mauris. Venenatis aliquet id ultrices a lacus. Pretium vehicula pretium posuere justo sed.", logo: logoIcon.ArtVenueIcon, avatar: artVenue, name: "David Oshodi", jobTitle: "Manager" },
]

export const articles = [
    { img: congue, title: "Congue velit risus", text: "Nunc gravida semper tellus neque scelerisque eget tincidunt in." },
    { img: ut, title: "Ut mauris", text: "Phasellus venenatis massa bibendum posuere dictum tristique." },
    { img: aliquam, title: "Aliquam tortor nunc", text: "Leo mollis fermentum praesent in condimentum id velit purus in." },
    { img: fusce, title: "Fusce non morbi", text: "In sed bibendum metus pretium cursus tellus pharetra." }
]

export const amazing = [
    { img: ipsum, title: "Varius est diam", subTitle: "Ipsum" },
    { img: purus, title: "Diam felis tempus", subTitle: "Purus" },
    { img: urna, title: "Vel nulla varius", subTitle: "Urna" },
    { img: vitae, title: "Amet tristique a", subTitle: "Vitae" },
]

export const footerData = {
    product: ["Pricing", "Overview", "Browse", "Accessibility", "Five"],
    solutions: ["Brainstorming", "Ideation", "Wireframing", "Research", "Design"],
    support: ["Contact Us", "Developers", "Documentation", "Integrations", "Reports"]
}

export const points = [
    { longitude: -100, latitude: 40 },
    { longitude: 116.4074, latitude: 39.9042 },
    { longitude: 2.3522, latitude: 48.8566 },
    { longitude: 31.2357, latitude: 30.0444 },
];